...
def pairwise_distance(data1, data2, batch_size=None):
    ...
    '''
    data1 = torch.from_numpy(data1).cuda()if isinstance(data1, np.ndarray) else data1
    data2 = torch.from_numpy(data2).cuda() if isinstance(data2, np.ndarray) else data2
    #N*1*M
...
class K_Means:
    ...
    def fit_mix_once(self, u_feats, l_feats, l_targets, random_state):
        u_feats =torch.tensor(u_feats).cuda()
        l_feats =torch.tensor(l_feats).cuda()
        l_targets =torch.tensor(l_targets).cuda()
        def supp_idxs(c):
